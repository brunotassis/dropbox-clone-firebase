class DropboxController {
        ...
        this.btnNewFolder.addEventListener('click', e => {
            ...
            if(name){
                this.getFirebaseRef().push().set({
                    name,
                    type: 'folder',
                    currenFolder: this.currenFolder.join('/')
                });
            }
        });
    ...
    initEventsLi(li) {
        li.addEventListener('click', e => {
            if (e.shiftKey) {
                ...
                this.fileListEl.dispatchEvent(this.onSelectionOnChange);
                return true;
            }

            if (!e.ctrlKey) {
                this.fileListEl.querySelectorAll('li.selected').forEach(e => {
        ...
        li.addEventListener('dblclick', e => {
            ...
            switch (file.type) {
                case 'folder':
                    this.currenFolder.push(file.name);
                    this.openFolder();
                break;
    ...
    renderNav(){
        ...
        for (let i = 0; i < this.currenFolder.length; i++) {
            const folderName = this.currenFolder[i];
            const span = document.createElement('span');

            path.push(folderName);

            if((i + 1) === this.currenFolder.length){
        ...
        this.navEl.querySelectorAll('a').forEach(a => {
            a.addEventListener('click', e => {
                e.preventDefault();
                this.currenFolder = a.dataset.path.split('/');
                this.openFolder();
            });
        });
    }
    ...
    getFirebaseRef(path) {
        if(!path) path = this.currenFolder.join('/');
    ...
    connectFirebase() {
        var config = {
            apiKey: "AIzaSyBzrLAfiElasXRs9u4auDL4MyEHTvHyEQ8",
            authDomain: "dropbox-clone-5505d.firebaseapp.com",
            databaseURL: "https://dropbox-clone-5505d.firebaseio.com",
            projectId: "dropbox-clone-5505d",
            storageBucket: "dropbox-clone-5505d.appspot.com",
        ...
        [...files].forEach(file => {
            promises.push(
                new Promise((resolve, reject) => {
                    let fileRef = firebase.storage().ref(this.currenFolder.join('/')).child(file.name);
                    let task = fileRef.put(file);

                    task.on('state_changed', snapshot => {
                        this.uploadProgress({
                            loaded: snapshot.bytesTranferred,
    ...
    uploadProgress(event, file) {
        ...
        let timeLeft = ((100 - percent) * timespant) / percent;
    ...
    formatTime(duration) {
        let seconds = parseInt((duration / 60) % 60);
    ...
    readFiles() {
        this.lastFolder = this.currenFolder.join('/');
    ...
    removeTask() {
        ...
        this.getSelection().forEach(li => {
            ...
            promises.push(
                new Promise((resolve, reject) => {
                    if(file.type === 'folder'){
                        this.removeFolderTask(this.currenFolder.join('/'), file.name).then(() => {
                            resolve({fields: {key}});
                        }).catch(err => {
                            reject(err);
                        });
                    }else if(file.type){
                        this.removeFile(this.currenFolder.join('/'), file.name).then(() => {
                            resolve({fields: {key}});
                        }).catch(err => {
                            reject(err);
                        });
